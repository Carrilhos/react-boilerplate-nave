const validarCPF = number => {
  ...
  if (
    ...
    return { valido: false, texto: 'Digite um CPF valido1' }
  for (i = 1; i <= 9; i++) sum = sum + parseInt(number.substring(i - 1, i)) * (11 - i)
  rest = (sum * 10) % 11

  if (rest == 10 || rest == 11) rest = 0
  if (rest != parseInt(number.substring(9, 10))) return { valido: false, texto: 'Digite um CPF valido2' }
  ...
  if (rest == 10 || rest == 11) rest = 0
  if (rest != parseInt(number.substring(10, 11))) return { valido: false, texto: 'digite um CPF valido3' }
